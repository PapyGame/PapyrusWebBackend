 ...
 */
@Service
public class DeletePrimitiveListItemEventHandler implements IFormEventHandler {
    ...
    public DeletePrimitiveListItemEventHandler(IFormQueryService formQueryService, ICollaborativeFormMessageService messageService, MeterRegistry meterRegistry) {
    ...
    public boolean canHandle(IFormInput formInput) {
        return formInput instanceof DeletePrimitiveListItemInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, Form form, IFormInput formInput) {
        ...
        if (formInput instanceof DeletePrimitiveListItemInput input) {
            ...
            if (optionalList.map(PrimitiveListWidget::isReadOnly).filter(Boolean::booleanValue).isPresent()) {
                ...
                status = optionalListItem.map(PrimitiveListItem::getDeleteHandler)
                        .map(Supplier::get)
                        .orElse(new Failure(""));
            }
