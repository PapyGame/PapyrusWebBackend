...
/**
 * The handler of the remove containment reference item event.
 *
 * @author Jerome Gout
 */
@Service
public class RemoveContainmentReferenceItemEventHandler implements IFormEventHandler {
    ...
    public boolean canHandle(IFormInput formInput) {
        return formInput instanceof RemoveContainmentReferenceItemInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, Form form, IFormInput formInput) {
        ...
        String message = this.messageService.invalidInput(formInput.getClass().getSimpleName(), RemoveContainmentReferenceItemInput.class.getSimpleName());
        IPayload payload = new ErrorPayload(formInput.id(), message);
        ChangeDescription changeDescription = new ChangeDescription(ChangeKind.NOTHING, formInput.representationId(), formInput);

        if (formInput instanceof RemoveContainmentReferenceItemInput input) {
            ...
            if (optionalReferenceWidget.map(ContainmentReferenceWidget::isReadOnly).filter(Boolean::booleanValue).isPresent()) {
                ...
                var optionalReferenceValue = optionalReferenceWidget
                        ...
                        .findFirst();
                status = optionalReferenceValue.map(ContainmentReferenceItem::getRemoveHandler)
                        .map(Supplier::get)
                        .orElse(new Failure(""));
            }
